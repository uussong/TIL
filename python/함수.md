...
- 함수(어떠한 구조,로직을 분해/추상화) 
  ...
  - input: 기본적으로 위치를 기반
    ...
    - 정의
      - 필수
      - 선택(기본값을 주어 처리)
      - 많이 받을 땐 *(tuple) **(dictionary)
  - output: 반드시 하나의 객체를 반환(return)0개? None 여러개? tuple
...
- 파이썬이 어떤 이름을 찾을 때 이름공간(namespace)을 찾음
  - namespace에서 이름을 찾을 때 Local->Enclosed->Global->Built-in 점점 큰 범위로 뚫고 찾아 나감
